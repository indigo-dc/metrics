...
\vspace{-3em}
\begin{center}
    \graybox{\strut SQA Progress Status}\redbox{\strut NOT COMPLETE (0\% done)} \\
\end{center}
...
\begin{ganttchart}[
    ...
    \ganttgroup[progress=0]{\textbf{\#3086} CLUES SQA FOR 1ST RELEASE}{1}{12} \\
            \ganttbar[
            progress=100,
            name=WBS1A
...
Repository exists under \texttt{indigo-dc} GitHub organization: \vspace{0.1em} \begin{center}\url{https://github.com/indigo-dc/clues-indigo.git}\end{center}


\input{title_codestyle.tex}
\label{sec:code_style}
\begin{tabular}{ll}
    Code style definition & \href{None}{None} \\
    Community/de-facto standard & \graybox{None} \\
    Exceptions & \graybox{None} \\
    Richness & \graybox{\strut None} \hspace{0.3em} \graybox{\strut Errors None} \graybox{\strut Warnings None} \href{None}{link}\\
\end{tabular}
\subsection{Build status}
Last build status on Jenkins CI \href{None}{None}.

\subsection{Observations}
\begin{enumerate}
        \item Progress is 50\% since the product is adhered to a standard but the job has not been defined (WP3 duty).
    \end{enumerate}
...
\subsection{Observations}
\begin{enumerate}
        \item Progress is 50\% since the product has implemented unit tests but the job has not yet been defined (WP3 duty).
    \end{enumerate}
